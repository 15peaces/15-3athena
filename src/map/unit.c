 ...
 * @return 1: Success 0: Fail
 *------------------------------------------*/
int unit_run(struct block_list *bl, struct map_session_data* sd, enum sc_type type)
{
...
int unit_resume_running(int tid, int64 tick, int id, intptr_t data)
{
	...
	if(sd && pc_iswugrider(sd))
		clif_skill_nodamage(ud->bl,ud->bl,RA_WUGDASH,ud->skill_lv,
			sc_start4(ud->bl,status_skill2sc(RA_WUGDASH),100,ud->skill_lv,unit_getdir(ud->bl),0,0,0));
	else
