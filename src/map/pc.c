...
static int pc_spiritball_attribute_timer(int tid, int64 tick, int id, intptr data)
{
	struct map_session_data *sd;
	int i;
...
int pc_cart_additem(struct map_session_data *sd,struct item *item_data,int amount)
{
	...
	if( itemdb_isstackable2(data) && !item_data->expire_time )
	{
		ARR_FIND( 0, MAX_CART, i,
			sd->cart.u.items_cart[i].nameid == item_data->nameid && sd->cart.u.items_cart[i].bound == item_data->bound &&
	...
	if (i < MAX_ITEMDELAYS) {
		if (sd->item_delay[i].nameid) {// found
			if (DIFF_TICK(sd->item_delay[i].tick, tick) > 0) {
				...
				if (e_tick > 99)
					sprintf(e_msg, msg_txt(802), // Item Failed. [%s] is cooling down. Wait %.1f minutes.
						itemdb_jname(sd->item_delay[i].nameid), (double)e_tick / 60);
				else
					sprintf(e_msg, msg_txt(803), // Item Failed. [%s] is cooling down. Wait %d seconds.
...
int pc_checkequip(struct map_session_data *sd,int pos, bool checkall)
{
	...
	for(i=0;i<EQI_MAX;i++){
		if (pos & equip_pos[i]) {
			if (checkall && (pos&~equip_pos[i]) != 0 && sd->equip_index[i] == -1) {
...
static void pc_gainexp_disp(struct map_session_data *sd, unsigned int base_exp, unsigned int next_base_exp, unsigned int job_exp, unsigned int next_job_exp, bool lost) {
	...
	sprintf(output, msg_txt(734), // Experience %s Base:%ld (%0.2f%%) Job:%ld (%0.2f%%)
		(lost) ? msg_txt(733) : msg_txt(732),
...
int pc_equipitem(struct map_session_data *sd, int n, int req_pos, bool equipswitch)
{
	...
	if( n < 0 || n >= MAX_INVENTORY ) {
		if (equipswitch) {
			clif_equipswitch_add(sd, n, req_pos, ITEM_EQUIP_ACK_OK);
		}
		else {
	...
	if(!pc_isequip(sd,n) || !(pos&req_pos) || sd->inventory.u.items_inventory[n].equip != 0 || sd->inventory.u.items_inventory[n].attribute==1 ) { // [Valaris]
		if (equipswitch) {
...
int pc_checkitem(struct map_session_data *sd)
{
	...
	{// check for invalid(ated) items
		for( i = 0; i < MAX_INVENTORY; i++ )
		{
			...
			{
				ShowWarning("Removed invalid/disabled item id %d from inventory (amount=%d, char_id=%d).\n", id, sd->inventory.u.items_inventory[i].amount, sd->status.char_id);
				pc_delitem(sd, i, sd->inventory.u.items_inventory[i].amount, 0, 0);
			}
		}
		...
		{
			id = sd->cart.u.items_cart[i].nameid;

			if( id && !itemdb_available(id) )
			{
				ShowWarning("Removed invalid/disabled item id %d from cart (amount=%d, char_id=%d).\n", id, sd->cart.u.items_cart[i].amount, sd->status.char_id);
				pc_cart_delitem(sd, i, sd->cart.u.items_cart[i].amount, 0);
			}
		}
	}
