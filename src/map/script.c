...
void script_hardcoded_constants(void)
{
	...
	/* instances */
	export_constant(IOT_NONE);
	export_constant(IOT_CHAR);
...
BUILDIN_FUNC(countinarray) {
	int case_count = 0;
	struct map_session_data* sd = NULL;
	DBMap** ref1 = NULL, ** ref2 = NULL;
	struct script_data* data1 = script_getdata(st, 2);
	...
	const char* name1 = reference_getname(data1);
	const char* name2 = reference_getname(data2);
	ref1 = reference_getref(data1);
	ref2 = reference_getref(data2);

	if (not_server_variable(*name1) && not_server_variable(*name2) && !script_rid2sd(st))
		return 1;

	const unsigned int array_size1 = getarraysize(st, reference_getid(data1), 0, is_string_variable(name1), ref1);
	const unsigned int array_size2 = getarraysize(st, reference_getid(data2), 0, is_string_variable(name2), ref2);
	...
	unsigned int i = reference_getindex(data1);
	unsigned int j = reference_getindex(data2);
	...
	const int id1 = reference_getid(data1);
	const int id2 = reference_getid(data2);
	...
	if (is_string_variable(name1) && is_string_variable(name2)) {
		...
		for (; i < array_size1; ++i) {
			...
			for (j = reference_getindex(data2); j < array_size2; j++) {
				const int64 temp2 = get_val2_num(st, reference_uid(id2, j), ref2);
...
BUILDIN_FUNC(makeitem)
{
	...
	if( data_isstring(data) ) {
		...
		if (val < 0) {
			flag = 1;
			nameid = itemdb_searchrandomid((t_itemid)(-1 * val));
		}
		else {
			nameid = (t_itemid)val;
		}
...
BUILDIN_FUNC(mergeitem2) {
	...
	if (script_hasdata(st, 2)) {
		...
		if (data_isstring(data)) {// "<item name>"
			...
			nameid = conv_num(st, data);
			if (!(id = itemdb_exists(nameid))) {
				ShowError("buildin_mergeitem2: Nonexistant Item %u requested.\n", nameid);
				script_pushint(st, count);
				return 1;
			}
		}
...
/*==========================================
 * 審efine +1 item at pos and log and display refine
 *------------------------------------------*/
...
static int buildin_getareadropitem_sub(struct block_list *bl,va_list ap) {
	t_itemid item = va_arg(ap, t_itemid);
...
BUILDIN_FUNC(consumeitem)
{
	...
	if( data_isstring( data ) ){
		...
		t_itemid nameid = conv_num( st, data );
...
BUILDIN_FUNC(callshop)
{
	...
	if( !nd || nd->bl.type != BL_NPC || (nd->subtype != NPCTYPE_SHOP && nd->subtype != NPCTYPE_CASHSHOP && nd->subtype != NPCTYPE_MARKETSHOP && nd->subtype != NPCTYPE_ITEMSHOP && nd->subtype != NPCTYPE_POINTSHOP) ) {
...
BUILDIN_FUNC(npcshopitem)
{
	...
	if( !nd || ( nd->subtype != NPCTYPE_SHOP && nd->subtype != NPCTYPE_CASHSHOP && nd->subtype != NPCTYPE_MARKETSHOP && nd->subtype != NPCTYPE_ITEMSHOP && nd->subtype != NPCTYPE_POINTSHOP ) ) { // Not found.
		script_pushint(st,0);
		return 0;
	}
...
BUILDIN_FUNC(npcshopadditem) {
	...
	if (nd->subtype == NPCTYPE_MARKETSHOP) {
		for (n = 0, i = 3; n < amount; n++, i += offs) {
			t_itemid nameid = script_getnum(st,i), j;
...
BUILDIN_FUNC(instance_create)
{
	...
	res = instance_create(owner_id, name, (enum instance_owner_type) type);
...
BUILDIN_FUNC(searchstores)
{
	...
	if( effect > 1 )
	{
		ShowError("buildin_searchstores: Invalid effect id %u, specified.\n", effect);
		return 1;
	}
