...
int status_check_skilluse(struct block_list *src, struct block_list *target, int skill_num, int skill_lv, int flag)
{
	...
	if (sc && sc->option)
	{
		...
		if (sc->option&OPTION_MADOGEAR && ((TBL_PC*)src)->skillitem != skill_num)
			switch(skill_num)
...
void status_calc_regen_rate(struct block_list *bl, struct regen_data *regen, struct status_change *sc)
{
	...
	if( sc->data[SC_GENTLETOUCH_REVITALIZE] ){
		const struct status_change_entry *sce = sc->data[SC_GENTLETOUCH_REVITALIZE];
	...
	switch(type)
	{
		...
		case SC__GROOMY:
			...
			{ // Removes Animals
				...
				if( pc_iswugrider(sd) ) pc_setoption(sd, sd->sc.option&~OPTION_WUG);
