...
ACMD_FUNC(jobchange)
{
	...
	if (job == 13 || job == 21 || job == 22 || job == 26 || job == 27 || job == 28 || job == 29 || job == 30 || job == 4014 || job == 4022 ||
	job == 4036 || job == 4044 || job == 4048 || (job >= JOB_RUNE_KNIGHT2 && job <= JOB_RUNE_KNIGHT_T6) ||
...
ACMD_FUNC(bodystyle)
{
	...
	if (!((sd->class_&MAPID_THIRDMASK) == MAPID_MECHANIC || (sd->class_&MAPID_THIRDMASK) == MAPID_GUILLOTINE_CROSS || (sd->class_&MAPID_THIRDMASK) == MAPID_GENETIC))
