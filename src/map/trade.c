...
void trade_tradeack(struct map_session_data *sd, int type)
{
	...
	if ((sd->npc_id != 0 && sd->state.using_megaphone == 0) || sd->state.vending != 0 || sd->state.prevend != 0
		|| sd->state.buyingstore != 0 || sd->state.storage_flag != 0
		|| (tsd->npc_id != 0 && tsd->state.using_megaphone == 0) || tsd->state.vending != 0 || tsd->state.prevend != 0
	|| tsd->state.buyingstore != 0 || tsd->state.storage_flag != 0) {
		//Fail
		clif_tradestart(sd, 2);
	...
	for(i = 0; i < 10; i++) {
		...
		{ // if more than the player have -> hack
			...
			if (battle_config.ban_hack_trade < 0) {
				chrif_req_login_operation(-1, sd->status.name, 1, 0); // type: 1 - block
			...
			// if we ban people
				...
				chrif_req_login_operation(-1, sd->status.name, 2, battle_config.ban_hack_trade * 60); // type: 2 - ban (year, month, day, hour, minute, second)
				set_eof(sd->fd); // forced to disconnect because of the hack
				// message about the ban
...
void trade_tradeadditem(struct map_session_data *sd, short index, short amount)
{
	...
	if( ((item->bound == BOUND_ACCOUNT || item->bound > BOUND_GUILD) || (item->bound == BOUND_GUILD && sd->status.guild_id != target_sd->status.guild_id)) && !pc_can_give_bounded_items(sd->gmlevel) ) { // Item Bound 
