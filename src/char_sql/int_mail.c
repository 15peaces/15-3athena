...
static int mail_fromsql(uint32 char_id, struct mail_data* md)
{
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `id` FROM `%s` WHERE `dest_id`='%d' AND `status` < 3 ORDER BY `id` LIMIT %d", mail_db, char_id, MAIL_MAX_INBOX + 1) ){
...
int mail_savemessage(struct mail_message* msg)
{
	...
	StringBuf_Init(&buf);
	StringBuf_Printf(&buf, "INSERT INTO `%s` (`send_name`, `send_id`, `dest_name`, `dest_id`, `title`, `message`, `time`, `status`, `zeny`,`type`", mail_db);
	...
	for( i = 0; i < MAIL_MAX_ITEM; i++ ){
		...
		StringBuf_Printf(&buf, "('%"PRIu64"', '%hu', '%d', '%u', '%d', '%d', '%d', '%d', '%"PRIu64"'", (uint64)msg->id, i, msg->item[i].amount, msg->item[i].nameid, msg->item[i].refine, msg->item[i].attribute, msg->item[i].identify, msg->item[i].bound, msg->item[i].unique_id);
		for (j = 0; j < MAX_SLOTS; j++)
...
static bool mail_loadmessage(int mail_id, struct mail_message* msg)
{
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `id`,`send_name`,`send_id`,`dest_name`,`dest_id`,`title`,`message`,`time`,`status`,`zeny`,`type` FROM `%s` WHERE `id` = '%d'", mail_db, mail_id )
	||  SQL_SUCCESS != Sql_NextRow(sql_handle) ){
		...
		if( msg->type == MAIL_INBOX_NORMAL && mail_return_days > 0 ){
			msg->scheduled_deletion = msg->timestamp + mail_return_days * 24 * 60 * 60;
		}else if( msg->type == MAIL_INBOX_RETURNED && mail_delete_days > 0 ){
			msg->scheduled_deletion = msg->timestamp + mail_delete_days * 24 * 60 * 60;
		}else{
	...
	StringBuf_Printf(&buf, " FROM `%s`", mail_attachment_db);
...
int mail_timer_sub( int limit, enum mail_inbox_type type ){
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `id`,`char_id`,`account_id` FROM `%s` `m` INNER JOIN `%s` `c` ON `c`.`char_id`=`m`.`dest_id` WHERE `type` = '%d' AND `time` <= UNIX_TIMESTAMP( NOW() - INTERVAL %d DAY ) ORDER BY `id` LIMIT %d", mail_db, char_db, type, limit, MAIL_MAX_INBOX + 1) ){
		Sql_ShowDebug(sql_handle);
		return 0;
	}
	...
	for( i = 0; i < MAIL_MAX_INBOX; i++ ){
		...
		map_fd = char_server_fd(mails[i].account_id);
...
int mail_return_timer( int tid, int64 tick, int id, intptr_t ptr ){
	return mail_timer_sub( mail_return_days, MAIL_INBOX_NORMAL );
}

int mail_delete_timer( int tid, int64 tick, int id, intptr_t data ){
	return mail_timer_sub( mail_delete_days, MAIL_INBOX_RETURNED );
}
...
static bool mail_DeleteAttach(int mail_id){
	if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `id` = '%d'", mail_attachment_db, mail_id ) ){
 		Sql_ShowDebug(sql_handle);
		return false;
	}
