...
//clan cache
static DBMap* clan_db_; // int clan_id -> struct clan*

int inter_clan_removemember_tosql(uint32 account_id, uint32 char_id){
	if( SQL_ERROR == Sql_Query( sql_handle, "UPDATE `%s` SET `clan_id` = '0' WHERE `char_id` = '%d'", char_db, char_id ) ){
...
struct clan* inter_clan_fromsql(int clan_id){
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `name`, `master`, `mapname`, `max_member` FROM `%s` WHERE `clan_id`='%d'", clan_db, clan_id) ){
		Sql_ShowDebug(sql_handle);
		return NULL;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `opposition`,`alliance_id`,`name` FROM `%s` WHERE `clan_id`='%d'", clan_alliance_db, clan_id) ){
		Sql_ShowDebug(sql_handle);
		aFree(clan);
		return NULL;
	}
...
int inter_clan_init(void){
	...
	if( SQL_ERROR == Sql_Query( sql_handle, "SELECT `clan_id` FROM `%s`", clan_db ) ){
		Sql_ShowDebug(sql_handle);
		return 1;
	}
